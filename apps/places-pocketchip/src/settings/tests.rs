...
/// PocketCHIP: the one authoritative fresh-install geometry is the device's own
/// 480x272 panel, and `Settings::default()` (and therefore a written settings
/// file) opens full screen at exactly that size. Nothing else may hard-code a
/// startup window dimension.
#[test]
fn test_the_default_window_is_the_device_panel() {
    assert_eq!(DEFAULT_WINDOW_WIDTH, 480);
    assert_eq!(DEFAULT_WINDOW_HEIGHT, 272);
    let settings = Settings::default();
    assert_eq!(settings.window_size(), (480, 272));
    assert_eq!(settings.window_mode(), WindowMode::Fullscreen);
    // The aspect is the panel's own 30:17, which is what the renderer's scene
    // target and its UI layout are calibrated against.
    assert_eq!(
        DEFAULT_WINDOW_WIDTH * 17,
        DEFAULT_WINDOW_HEIGHT * 30,
        "the default must be exactly the panel's aspect"
    );
}
...
fn test_legacy_settings_files_receive_modern_defaults() {
    ...
    assert_eq!(parsed.window_mode(), WindowMode::Fullscreen);
    assert_eq!(parsed.window_size(), (480, 272));
}
...
fn test_new_preferences_persist_round_trip() {
    ...
    assert_eq!(loaded.quality_profile(), QualityProfile::Low);
...
fn test_startup_overrides_outrank_saved_values_until_changed() {
    ...
    assert!(apply.graphics_rebuild && !apply.vsync);
}
...
fn test_graphics_settings_are_independent() {
    ...
    // Start from the profile the player is not on, so the step is a real change.
    assert!(settings.set_quality(QualityProfile::Full));
    ...
    // Lightmaps are a player setting, not a profile term, and the two never
    // rewrite each other.
...
fn test_reselecting_the_same_value_is_inert() {
    ...
    assert!(!settings.set_window_mode(WindowMode::Fullscreen));
...
/// `restore_defaults` returns every preference to the PocketCHIP default and
/// asks every subsystem to re-read it.
#[test]
...
/// The `LIMINAL_NO_LIGHTMAPS` variable names the stage to turn off, so a
/// truthy value must *disable* lightmaps. This is the contract the benchmark
/// scripts and `docs/POCKETCHIP.md` document; pinning it here keeps the name
/// and the effective state from drifting apart again.
#[test]
fn test_the_no_lightmaps_override_disables_the_stage_when_truthy() {
    assert_eq!(
        lightmaps_override(None),
        None,
        "unset leaves the saved value"
    );
    for off in ["0", "false", "no", "off", "", "  FALSE  "] {
        assert_eq!(
            lightmaps_override(Some(off)),
            Some(true),
            "{off:?} must keep lightmaps on"
        );
    }
    for on in ["1", "true", "yes", "on", "anything"] {
        assert_eq!(
            lightmaps_override(Some(on)),
            Some(false),
            "{on:?} must turn lightmaps off"
        );
    }
}
