...
/// Smallest cell size, in metres, the tuning override will accept.
///
/// A guard against a nonsense `LIMINAL_CELL_METRES`, not a policy: the shipped
/// grid is [`SPATIAL_SINGLE_CELL_METRES`].
pub const SPATIAL_CELL_MIN_METRES: f32 = 12.0;

/// Largest cell size, in metres, a level is partitioned with.
///
/// `PocketCHIP`: there is no partition. The report below measured why — the
/// spatial split only ever culled about 6 % of the level's vertices, because
/// the demo's rooms are smaller than the 12 m minimum cell and its long
/// corridors are seen end-on, while it multiplied the static draw batches by
/// roughly 2.5x. A draw call on this driver costs a fixed per-submission price
/// in the single-CPU box that prepares it, so trading 94 % of the geometry for
/// a fifth of the submissions is the wrong side of the trade.
///
/// One cell therefore covers the level: every face that shares a surface group
/// lands in the same batch and is submitted once. `LIMINAL_CELL_METRES` still
/// overrides the size for a benchmark sweep.
pub const SPATIAL_SINGLE_CELL_METRES: f32 = 4_096.0;

/// Per-axis world-space cell grid anchored at the world origin.
///
/// The grid decides how many static draw batches a level produces: one batch
/// per (surface group, cell). It is a partition, so the batch count is bounded
/// by `cells x surface kinds`, and the cell size is the culling-granularity end
/// of the trade — a coarser grid culls less geometry and submits fewer batches.
/// See [`SPATIAL_SINGLE_CELL_METRES`] for the `PocketCHIP` setting.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct CellGrid {
    pub x: f32,
    pub z: f32,
...
impl CellGrid {
    ...
    pub fn uniform(cell_metres: f32) -> Self {
        let size = if cell_metres.is_finite() && cell_metres >= SPATIAL_CELL_MIN_METRES {
            cell_metres.min(SPATIAL_SINGLE_CELL_METRES)
        } else {
            SPATIAL_SINGLE_CELL_METRES
        };
        Self { x: size, z: size }
    }

    /// The grid a level of this extent is partitioned with.
    ///
    /// The extent is not consulted: see [`SPATIAL_SINGLE_CELL_METRES`]. It
    /// stays a parameter so a future target with headroom for culling can size
    /// the grid from the level again without changing the call site.
    #[must_use]
    pub fn for_extent(_extent_x: f32, _extent_z: f32) -> Self {
        Self::uniform(SPATIAL_SINGLE_CELL_METRES)
    }

    /// The cell containing `point`. Non-finite coordinates fall back to cell
    /// (0, 0) rather than producing an unusable key.
    ///
    /// A single-cell grid always answers cell (0, 0): the level is one batch
    /// group regardless of where its geometry sits relative to the world
    /// origin, which is what makes the whole level one batch per surface group
    /// even for a level that straddles the origin.
    #[must_use]
    pub fn cell_of(&self, point: [f32; 3]) -> CellKey {
        if self.is_single_cell() {
            return CellKey { x: 0, z: 0 };
        }
        CellKey {
    ...
    /// True when this grid does not partition the level.
    #[must_use]
    pub fn is_single_cell(&self) -> bool {
        self.x >= SPATIAL_SINGLE_CELL_METRES && self.z >= SPATIAL_SINGLE_CELL_METRES
    }
    ...
    pub fn describe(&self) -> String {
        if self.is_single_cell() {
            return "level-wide batch (no spatial split)".to_string();
        }
