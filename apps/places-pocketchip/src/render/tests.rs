...
#[test]
fn a_level_wide_batch_is_one_submission_and_the_frustum_still_rejects_far_bounds() {
    // PocketCHIP: the level is one cell, so the two clusters — which share
    // materials — are submitted as one batch each rather than one per cell.
    // The cull the cell split used to buy is worth about 6 % of the demo's
    // vertices; the extra submissions it costs are the dominant frame cost on
    // this GPU, so the trade is explicitly the other way.
    //
    // What still has to hold is the predicate that decides whether a batch is
    // submitted at all: the frustum's AABB test, checked here on the real
    ...
    // Bounds of everything the mesh placed on each side of the camera.
    let side_bounds = |negative_z: bool| {
        let mut aabb = crate::spatial::Aabb::EMPTY;
        for batch in &mesh.ranges {
            for vertex in &batch.vertices {
                if (vertex.pos[2] < 0.0) == negative_z {
    ...
    let negative = side_bounds(true);
    let positive = side_bounds(false);
    assert!(!negative.is_empty() && !positive.is_empty());

    assert!(
        toward_far.intersects_aabb(&negative),
        "the -Z cluster is in front of a yaw-0 camera"
    );
    assert!(
        !toward_far.intersects_aabb(&positive),
        "a yaw-0 camera must reject the +Z cluster"
    );
    assert!(
        toward_near.intersects_aabb(&positive),
        "the +Z cluster is in front of a yaw-180 camera"
    );
    assert!(
        !toward_near.intersects_aabb(&negative),
        "a yaw-180 camera must reject the -Z cluster"
    );

    // Both clusters are inside the level-wide batch the renderer would submit,
    // which is what makes the single-cell grid one draw call per surface group.
    let mut whole = crate::spatial::Aabb::EMPTY;
    for batch in &mesh.ranges {
        for vertex in &batch.vertices {
            whole.expand(vertex.pos);
        }
    }
    assert!(!whole.is_empty());
    for axis in 0..3 {
        assert!(whole.min[axis] <= negative.min[axis] && whole.min[axis] <= positive.min[axis]);
        assert!(whole.max[axis] >= negative.max[axis] && whole.max[axis] >= positive.max[axis]);
    }
    assert!(visible_static_vertices(&mesh, &toward_far) > 0);
}
...
#[test]
fn the_real_prop_batches_carry_bounds_and_cover_every_placement() {
    let catalog = shipped_catalog();
    let mut assets = shipped_assets();
    // Two chairs 40 m apart share one level-wide batch, because the PocketCHIP
    // build partitions the level into a single cell: the pair would otherwise
    // cost two draw submissions to save a cull that the demo's own room sizes
    // show is worth about 6 % of the geometry. The batch must still carry
    // bounds that contain every vertex of both placements.
    let level = level_with_wall(
        "[]",
    ...
    assert_eq!(
        batches.len(),
        1,
        "one batch per model, not per (model, cell)"
    );
    ...
    // Both placements must be inside the one batch's bounds, one either side of
    // the origin.
    let bounds = batches[0].bounds;
    assert!(bounds.min[0] < -15.0, "near cluster missing: {bounds:?}");
    assert!(bounds.max[0] > 15.0, "far cluster missing: {bounds:?}");
}
...
use super::renderer::{
    ...
    batch_pass_for, collect_translucent_draws, pack_static_batches,
};
...
#[test]
fn every_shader_source_has_balanced_preprocessor_conditionals() {
    // The material tiers are built by wrapping sections of one GLSL body in
    // `#ifndef LIMINAL_MATERIAL_SIMPLE`, and the two alpha variants add
    // `#ifdef ALPHA_CUTOUT` on top. An unbalanced pair is not a subtle shading
    // bug: the driver refuses to compile the program and the renderer cannot
    // start at all, which is exactly what a missing `#endif` did once. Check
    // every source the renderer actually hands to the compiler.
    let sources = [
        (
            "world vertex (full)",
            super::view::vertex_shader_source(false),
        ),
        (
            "world vertex (simple)",
            super::view::vertex_shader_source(true),
        ),
        (
            "world fragment (full)",
            super::view::fragment_shader_source(false, false),
        ),
        (
            "world fragment (cutout)",
            super::view::fragment_shader_source(true, false),
        ),
        (
            "world fragment (simple)",
            super::view::fragment_shader_source(false, true),
        ),
        (
            "world fragment (simple cutout)",
            super::view::fragment_shader_source(true, true),
        ),
        (
            "decal fragment",
            super::view::DECAL_FRAGMENT_SHADER_SRC.to_string(),
        ),
    ];
    for (name, source) in sources {
        ...
        for (index, line) in source.lines().enumerate() {
            let token = line.trim();
            // `#ifdef`, `#ifndef` and `#if` all open a level; `#elif` does not.
            if token.starts_with("#if") {
                depth += 1;
            } else if token.starts_with("#endif") {
                depth -= 1;
            }
            assert!(depth >= 0, "{name}: unmatched #endif at line {}", index + 1);
        }
        assert_eq!(depth, 0, "{name}: {depth} unterminated conditional(s)");
    }
}

#[test]
fn the_two_material_tiers_are_one_body_under_a_tier_define() {
    // The reduced tier is the same GLSL body with `LIMINAL_MATERIAL_SIMPLE`
    // defined, so the tier is a property of the source every program shares
    // rather than of a uniform the fragment stage has to test. The parts the
    // reduced tier exists to drop must therefore sit inside the guard, and the
    // fog substitution must be a guarded pair. The compile-out itself is checked
    // on the device (the driver is the only thing that preprocesses GLSL); what
    // is pinned here is that the guard still covers what it is supposed to.
    let body_start = super::view::fragment_shader_source(false, false);
    let simple = super::view::fragment_shader_source(false, true);
    let full = super::view::fragment_shader_source(false, false);
    let simple_vertex = super::view::vertex_shader_source(true);
    let full_vertex = super::view::vertex_shader_source(false);

    assert!(simple.starts_with("#define LIMINAL_MATERIAL_SIMPLE 1"));
    assert!(!full.contains("#define LIMINAL_MATERIAL_SIMPLE"));
    assert!(simple_vertex.starts_with("#define LIMINAL_MATERIAL_SIMPLE 1"));
    assert!(!full_vertex.contains("#define LIMINAL_MATERIAL_SIMPLE"));

    // The surface frame is declared and interpolated only under the guard.
    for source in [full.as_str(), simple_vertex.as_str()] {
        assert!(source.contains("#ifndef LIMINAL_MATERIAL_SIMPLE"));
    }
    assert!(full_vertex.contains("attribute vec3 a_normal"));

    // Every guarded section is closed, and the fog keeps one exact and one
    // cheap branch. (`every_shader_source_has_balanced_preprocessor_conditionals`
    // owns the balance itself.)
    let guards = |text: &str| text.matches("#ifndef LIMINAL_MATERIAL_SIMPLE").count();
    assert!(
        guards(&simple) >= 2,
        "the surface frame and the material sum are both guarded"
    );
    assert!(simple.contains("#ifdef LIMINAL_MATERIAL_SIMPLE"));
    // The reduced tier receives its fog as one interpolated float; the vertex
    // stage that produces it carries the cheap curve, and the full tier keeps
    // the exact one.
    assert!(
        simple.contains("v_fog_amount"),
        "the reduced fragment stage must read the interpolated fog amount"
    );
    assert!(
        simple_vertex.contains("x_sq / (1.0 + x_sq)"),
        "the reduced vertex stage must carry the rational fog curve"
    );
    assert!(
        full.contains("1.0 - exp(-x_sq)"),
        "the full tier must keep the exact fog curve"
    );
    // No fragment stage takes a square root any more: the full tier evaluates
    // its curve from the squared distance.
    for source in [simple.as_str(), full.as_str()] {
        assert!(
            !source.contains("length(u_camera_pos") && !source.contains("sqrt("),
            "fog must use the squared distance"
        );
    }
    assert!(
        body_start.len() == full.len(),
        "the full tier is the body unchanged"
    );
}

#[test]
fn the_world_fragment_shader_gates_the_lightmap_reads() {
    // The stacked atlas is one sampler bound to one unit for every draw, so the
    // fragment must only sample it when it actually takes its light from it:
    // the global switch and the vertex's own page byte both have to agree
    // before `texture2D` runs. This is the shader half of the lightmap-unit
    // invariant; the binding half is `Renderer::bind_lightmap_units`.
    let source = super::view::fragment_shader_source(false, false);
    let guard = source
        .find("if (lightmap_on > 0.5)")
        .expect("the atlas path must stay behind the lightmap guard");
    let atlas = source
        .find("texture2D(u_lightmap,")
        .expect("the stacked atlas must stay declared");
    assert!(
        guard < atlas,
        "the atlas read must happen after the guard opens"
    );
    assert_eq!(
        source.matches("texture2D(u_lightmap,").count(),
        1,
        "the stacked atlas must be read exactly once"
    );
...
fn the_lightmap_units_cover_every_declared_atlas_sampler() {
    use super::view::{LIGHTMAP_PAGE_SLOTS, LIGHTMAP_TEXTURE_UNIT};

    // One bound unit for the whole stacked atlas.
    assert_eq!(LIGHTMAP_TEXTURE_UNIT, 2, "the atlas unit is unit 2");
    // The lighting side plans against the same bound: a bake that may need a
    // page the renderer cannot stack must fall back, not drop it.
    assert_eq!(LIGHTMAP_ATLAS_MAX_PAGES, LIGHTMAP_PAGE_SLOTS);

    // The world stage declares exactly one atlas sampler, and the vertex stage
    // is what folds the page byte into it.
    let source = super::view::fragment_shader_source(false, false);
    ...
    assert_eq!(
        atlas_samplers, 1,
        "the world stage declares one sampler for the stacked atlas"
    );
    let vertex = super::view::vertex_shader_source(false);
    assert!(
        vertex.contains("u_lightmap_page_scale"),
        "the vertex stage must scale the page byte into the stacked texture"
    );
}
