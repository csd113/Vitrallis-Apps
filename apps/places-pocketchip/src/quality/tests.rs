...
#[test]
fn the_shipping_profile_is_the_reduced_one() {
    // PocketCHIP: the reduced profile is what the device ships, not an
    // optional downgrade a player has to find. `Full` survives only as the A/B
    // reference a benchmark run can still ask for.
    assert_eq!(QualityProfile::default(), QualityProfile::Low);
    assert_eq!(QualityProfile::DEFAULT, QualityProfile::Low);
    assert!(QualityProfile::DEFAULT.reduces_optional_features());
    assert!(!QualityProfile::DEFAULT.draws_surface_response());
