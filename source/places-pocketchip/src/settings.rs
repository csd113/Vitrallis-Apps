...
//! * the **startup overrides** (`LIMINAL_QUALITY=low`,
//!   `LIMINAL_NO_LIGHTMAPS=1`, ...), which are session-only and never
//!   persisted;
...
/// Width a fresh installation opens with.
///
/// `PocketCHIP`: the panel is 480x272 and the game is a full-screen handheld
/// experience, so the default window *is* the device's own mode. A fresh
/// installation therefore comes up at the right size without depending on the
/// display work area being queried successfully first, and a windowed fallback
/// lands on the same geometry the full-screen mode uses.
pub const DEFAULT_WINDOW_WIDTH: u32 = 480;
/// Height a fresh installation opens with.
pub const DEFAULT_WINDOW_HEIGHT: u32 = 272;

/// Smallest window edge a persisted or selected size may specify.
///
/// `PocketCHIP`: below the device's own 272-pixel panel height, so the panel mode
/// is always representable and `Settings::default()` survives a sanitize
/// unchanged. The desktop value of 320 was larger than the handheld's screen.
pub const MIN_WINDOW_EDGE: u32 = 200;
...
impl SettingsApply {
    ...
    pub const fn any(self) -> bool {
        self.graphics_rebuild || self.vsync || self.window
    }
}
...
// The booleans are independent player preferences (VSync and lightmaps),
...
/// The window mode a fresh installation opens in.
///
/// `PocketCHIP`: full screen. The device has no window manager and one 480x272
/// panel, so a decorated window would only cost drawable pixels and place the
/// game behind a title bar it can never be dragged out from under.
fn default_window_mode() -> String {
    WindowMode::Fullscreen.name().to_string()
}
...
/// Interprets a `LIMINAL_NO_LIGHTMAPS` value as the effective lightmaps state.
///
/// The variable names the stage to turn *off*: any value except empty, `0`,
/// `false`, `no` or `off` asks for a vertex-lit build for this process, so the
/// returned override is `Some(false)`. A missing variable is `None`, which
/// leaves the saved preference in force.
fn lightmaps_override(value: Option<&str>) -> Option<bool> {
...
impl Settings {
    ...
    pub fn apply_startup_overrides(&mut self, vsync: Option<bool>) {
        self.overrides = StartupOverrides {
            ...
            lightmaps: lightmaps_override(std::env::var(NO_LIGHTMAPS_OVERRIDE_ENV).ok().as_deref()),
            vsync,
        };
    }
