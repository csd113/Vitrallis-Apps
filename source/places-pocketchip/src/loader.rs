...
impl LevelManager {
    #[must_use]
    pub fn new() -> Self {
        Self::with_quality(QualityProfile::DEFAULT)
    }

    /// Builds a manager whose decoded-texture cache keeps only the pixels the
    /// given quality profile can upload.
    ///
    /// See [`crate::materials::TextureCache::with_sheet_budget`]: the session
    /// cache is where a decoded 1024x1024 sheet would otherwise live for the
    /// whole run next to the smaller texture the GPU actually holds.
    #[must_use]
    pub fn with_quality(profile: QualityProfile) -> Self {
        let assets_dir = crate::assets::resolve_asset_root().map_or_else(
            || PathBuf::from("assets/levels"),
            |root| root.join("levels"),
        );
        let budget = crate::quality::resident_sheet_budget(profile);
        let mut manager = Self {
            ...
            texture_cache: RefCell::new(TextureCache::with_sheet_budget(budget)),
        };
